...
def return_args(root_dir, filename, lesion_load, num_lesions, lesion):
    ...
    mask = torch.where(mask > 0.5, torch.tensor(1.0), torch.tensor(-1.0))  # Binarize the mask
    ...
    for batch_idx in tqdm(range(0, len(df), batch_size)):
        ...
        for _, row in batch_df.iterrows():
            model_kwargs = return_args(
                ...
                lesion_load=row['lesion_load'] / LESION_LOAD_MAX,
                num_lesions=row['num_lesions'] / NUM_LESION_MAX,
                lesion=row['lesion']
            )
        ...
        with torch.no_grad():
            samples = sample_fn(
                model,
                (batch_size, 2, 256, 256),  # Batch size and image size
                clip_denoised=True,
