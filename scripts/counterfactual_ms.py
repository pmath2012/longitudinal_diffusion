...
def get_random_lesion_info(df, batch_size):
    ...
    num_lesions = random_rows['num_lesions'].astype(float).values / NUM_LESION_MAX**2  # Scale based on max value during training
    lesion_load = random_rows['lesion_load'].astype(float).values / LESION_LOAD_MAX**2  # Scale based on max value during training
...
def preprocess_batch(batch, cond, df, microbatch=-1):
    ...
    changes_dict = {
            "lesion_load": {"original": model_kwargs["lesion_load"], "modified": cf_model_kwargs["lesion_load"]},
            "num_lesions": {"original": model_kwargs["num_lesions"], "modified": cf_model_kwargs["num_lesions"]},
            "lesion": {"original": model_kwargs["lesion"], "modified": cf_model_kwargs["lesion"]}
        }
    # Return the processed microbatch and the kwargs dictionary
    return micro_batch, model_kwargs, cf_model_kwargs, changes_dict
...
def main():
    ...
    while len(all_images) * args.batch_size < args.num_samples:
        ...
        batch, model_kwargs, cf_model_kwargs, changes_dict = preprocess_batch(batch, cond, df)
    ...
    cng = []
    for change_set in all_dos:
        cng.append(change_set)
    ...
    if dist.get_rank() == 0:
        ...
        if args.class_cond:
            ...
            np.savez(out_path, cfs, img, msk, cng)
